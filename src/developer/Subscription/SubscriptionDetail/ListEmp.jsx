...
function ListEmp({ dataDetail, subscriptionId, typePortal }) {
	const { configTable, filterLocal, onChangeOneParam } = usePaginationLocal(
		async (params) => {
			const res = await SubscriptionDev.getListEmpBill(subscriptionId, dataDetail.serviceId, typePortal, params);
			return res;
		},
		['name', 'status'],
		{
	...
	function checkDisplayTable() {
		if (
			(dataDetail.subscriptionStatus === 'IN_TRIAL' || dataDetail.subscriptionStatus === 'FUTURE') &&
		...
		if (
			...
			return true;
		if (dataDetail.subscriptionStatus !== 'IN_TRIAL' && dataDetail.subscriptionStatus !== 'FUTURE') return true;
		return false;
	}
