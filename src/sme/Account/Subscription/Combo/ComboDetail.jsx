...
function ComboDetail({ setHaveError }) {
	const { id } = useParams();
	const { getComboSubscriptionCombo } = SMESubscription;

	const { pathname } = useLocation();
	const pathToList = dropRight(pathname.split('/')).join('/');

	const { refetch, data: subscriptionInfo, isFetching } = useQuery(
		['getComboSubscriptionById', id],
		async () => {
			try {
				const res = await getComboSubscriptionCombo(id);
				return res;
			} catch (e) {
	...
	return (
		<CommonSubscription
			dataSubscription={subscriptionInfo}
