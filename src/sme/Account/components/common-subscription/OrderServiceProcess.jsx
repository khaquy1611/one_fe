...
import { Avatar, Button, message, Modal, Steps, Table, Tag } from 'antd';
...
function OrderServiceProcess({ orderServiceProcess, subId, refetch }) {
	const {
		createAt,
		updateAt,
		timeLine,
	...
	const currentStep = SMESubscription.orderServiceProcessStep.findIndex((x) => x.key === statusOrderId) || 0;
		...
		<>
			<CustomSteps current={currentStep}>
				{steps.map((x, index) => {
					...
					return (
						<Step className={`step-${index}`} title={tFilterField('orderServiceProcessOptions', x.title)} />
					);
				})}
			</CustomSteps>

			<div className="box-detail mt-8">
				<div className="mb-4 text-primary font-bold">Chi tiết đơn hàng</div>
				<div className="text-gray-80">Cập nhật lúc: {DX.formatTime(updateAt || createAt)}</div>
				<Table className="mt-4" columns={columns} dataSource={timeLine} pagination={false} />
			</div>

			<div className="box-detail">
				<div className="mb-4 text-primary font-bold">Trạng thái thanh toán</div>
				{renderPaymentStatus()}
			</div>

			<div className="box-detail">
				<div className="mb-4 text-primary font-bold">Thông tin đơn hàng</div>
				{renderBillInfo()}
			</div>
