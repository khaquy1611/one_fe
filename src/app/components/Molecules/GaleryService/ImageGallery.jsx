...
class ImageGallery extends React.Component {
	constructor(props) {
		...
		this.slideToIndex = throttle(this.unthrottledSlideToIndex, props.slideDuration, { trailing: false });
    ...
    */
	...
	handleThumbnailSwiping({ event, absX, absY, dir }) {
		...
		const { thumbsSwipedTranslate, thumbnailsWrapperHeight, thumbnailsWrapperWidth } = this.state;
		const thumbsElement = this.thumbnails && this.thumbnails.current;
	...
	renderItem(item) {
		...
		return (
			<div className="wrap-inner">
				{item.imageSet ? (
					<picture onLoad={(event) => this.handleImageLoaded(event, item)} onError={handleImageError}>
