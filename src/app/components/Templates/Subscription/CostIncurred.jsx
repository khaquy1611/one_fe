...
function CostIncurred({ dataPreviewCost = {} }) {
	const dataOldBill = dataPreviewCost.oldBill;

	const dataAllBill = !isEmpty(dataPreviewCost?.allBill) && dataPreviewCost.allBill?.costIncurred;
	const columnAllBillIncurred = renderColumnType(dataAllBill, COST.ALL_BILL).filter((el) => !el.hide);

	const dataCostIncurred = !isEmpty(dataPreviewCost?.costIncurred) && dataPreviewCost.costIncurred.costIncurred;
	const columnCostIncurred = renderColumnType(dataCostIncurred, COST.INCURRED).filter((el) => !el.hide);

	return (
		<>
			{!isEmpty(dataOldBill) && (
				<div className="border border-solid border-gray-100 rounded-xl p-4 mb-4">
					<div className="text-primary font-bold mb-4">Thông tin hoá đơn đã phát sinh</div>
					{!isEmpty(dataOldBill) &&
						dataOldBill.map(
							(item, index) =>
								!isEmpty(item) && (
									<div key={`${index + 1}`}>
										<div className="font-medium mb-4">Thông tin hóa đơn lần {item.times}</div>
										<Table
											scroll={{ x: 800 }}
											...
											summary={() => (
												<RenderSummary
													summaryBill={getTotalRow(
														renderColumnType(item.costIncurred).filter((el) => !el.hide),
														item.costIncurred,
													)}
												/>
											)}
			...
			<div className="border border-solid border-gray-100 rounded-xl p-4 mb-4">
				<div className="text-primary font-bold mb-4">Chi tiết tính tiền (dự kiến cho cả chu kỳ)</div>
				<Table
					scroll={{ x: 800 }}
					columns={columnAllBillIncurred}
					dataSource={dataAllBill}
					pagination={false}
					summary={() => <RenderSummary summaryBill={getTotalRow(columnAllBillIncurred, dataAllBill)} />}
				/>
			</div>

			{dataCostIncurred.length > 0 && (
				<div className="border border-solid border-gray-100 rounded-xl p-4">
					<div className="text-primary font-bold mb-4">Chi phí dự tính phát sinh</div>
					<Table
						scroll={{ x: 800 }}
						columns={columnCostIncurred}
						dataSource={dataCostIncurred}
						pagination={false}
						summary={() => (
							<RenderSummary summaryBill={getTotalRow(columnCostIncurred, dataCostIncurred)} />
						)}
					/>
				</div>
			)}
