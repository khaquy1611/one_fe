...
const ModalChooseAddon = ({
	...
	const reqParams = {
		...
		popUpType: 'COMBO',
		typeId: pricingInfo.comboPlanId,
		// addonIdsNot:
		// 	typeSupscrip !== 'create' && dataDetail.status === 'ACTIVE'
		// 		? addonsList.filter((el) => el.canNotDelete)?.map((el) => el.id) || []
		// 		: [],
	...
	const COLUMN_RIGHT = [
		...
		},
		{
			...
			//	dataIndex: 'tin',
			render: (text, record) =>
				`${record.bonusType !== 'ONCE' ? `${record.bonusValue}  ${TYPE[record?.type]} ` : `Một lần`}`,
			sorter: true,
		},
		{
			title: 'Nhà phát triển',
