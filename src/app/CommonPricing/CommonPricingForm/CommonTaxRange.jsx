...
export default function CommonTaxRange({
	...
	const onChangeSetupFee = (value) => {
		...
		if (parseInt(temp, 10) > 0 && isEmpty(form.getFieldValue('setupFeeTaxList'))) setDisableBtnTax(false);
		else setDisableBtnTax(true);

		if (!value) form.setFieldsValue({ setupFeeTaxList: [] });
	};

	const titleField = pathname.indexOf('addon') === -1 ? 'gói dịch vụ' : 'dịch vụ bổ sung';
		...
		<div className="relative mt-10">
			<Row>
				<Col span={6} className="font-semibold text-right pr-2 mt-2.5">
					...
					>
						{!setupFeeInfo ? `${tField('taxInfo')} ${titleField}` : tField('setupFeeInfo')}
					</p>
				</Col>
				<Col span={10}>
					<Item
					...
					>
						{() => {
							...
							if (setupFeeTaxLists.length === 0) {
								form.setFieldsValue({ hasTaxSetupFee: false });
							}
							return (
								((!!taxList.length && !setupFeeInfo) ||
									(!!setupFeeTaxLists.length && setupFeeInfo)) && (
									<Item
										name={!setupFeeInfo ? 'hasTax' : 'hasTaxSetupFee'}
										className="m-0"
			...
			{setupFeeInfo && (
				...
				>
					<Input
						suffix={
							objectCheck.setupFee && (
								<span>
									<Tooltip placement="topRight" title={objectCheck.setupFee}>
