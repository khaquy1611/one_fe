...
export default function HistoryDetailAdmin(portal) {
	...
	const { data: initValues } = useQuery(
		...
		async () => {
			const res = await AdminCombo.getDetailComboByComboId(historyId);
	...
	const breadcrumb = [
		...
		},
		{
			...
			url: !historyId ? '' : DX.admin.createPath(`/combo/${id}`),
		},
		{
			isName: true,
	...
	const goBack = () => {
		history.push(DX.admin.createPath(`/combo/${id}?tab=4`));
	};
