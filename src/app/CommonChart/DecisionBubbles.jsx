...
function DecisionBubbles(id, data, graph) {
	let showNodes = [];
	let showEdges = [];
	let curShowNodes = [];
	let curShowEdges = [];
	...
	const height = document.getElementById(id).scrollHeight || 500;
	...
	const mapNodeSize = (nodess, propertyName, visualRange) => {
		let minp = 9999999999;
		let maxp = -9999999999;
		nodess.forEach((node) => {
	...
	const layoutCfg = {
		...
		onTick: () => {
			...
			const heightz = graph.get('height');
			const widthz = graph.get('width');
			const padding = 10;
			nodeItems.forEach((item) => {
				const model = item.getModel();
				if (model.x > widthz - padding) model.x = widthz - padding;
				else if (model.x < padding) model.x = padding;

				if (model.y > heightz - padding) model.y = heightz - padding;
	...
	G6.registerNode(
		'bubble',
		{
			drawShape(cfg, group) {
				...
				keyShape.animate(
					() => {
						const pathz = self.getBubblePath(r, spNum, directions, rs);
						return { pathz };
					},
					{
						repeat: true,
				...
				mask.animate(
					() => {
						const pathz = self.getBubblePath(r, spNum, directions2, rs2);
						return { pathz };
					},
					{
						repeat: true,
	...
	// }else{
	// 	localStorage.setItem("dataChange", JSON.stringify(data));
	// }

	const loadData = (isRender, dataChange) => {
		...
		nodes = dataChange ? dataChange.nodes : data.nodes;
		edges = dataChange ? dataChange.edges : data.edges;
		showNodes = [];
		showEdges = [];
		nodeMap = new Map();
		edgesMap = new Map();
	...
	graph.on('node:mouseenter', (e) => {
		...
		if (!model.isLeaf && model.level !== 0) {
			fTag = model.tag;
			nodeItems.forEach((itemz) => {
				const itemModel = itemz.getModel();
				if (!itemModel.isLeaf) return;
				const modelTags = itemModel.tags;
				modelTags.forEach((mt) => {
					const mts = mt.split('-');
					if (mts[1] === fTag) {
						graph.setItemState(itemz, 'dark', false);
		...
		nodeItems.forEach((itemz) => {
			const node = itemz.getModel();
			if (findTagsMap.get(node.tag) !== undefined) {
				graph.setItemState(itemz, 'dark', false);
				node.light = true;
			}
		});
		edgeItems.forEach((itemz) => {
			const source = itemz.getSource().getModel();
			const target = itemz.getTarget().getModel();
			if (source.light && target.light) {
				graph.setItemState(itemz, 'dark', false);
			} else {
				graph.setItemState(itemz, 'dark', true);
			}
		});
		graph.paint();
	...
	// 	.then((res) => res.json())
	// 	.then((data) => {
	// 		loadData(services.totalServiceByDeveloper);
	// 		console.log(
	// 			"totalServiceByDeveloper",
	// 			services.totalServiceByDeveloper
	// 		);
	...
	// 	// loadData(false, JSON.parse(localStorage.getItem("dataChange")));
	// 	curShowNodes = [];
	// 	curShowEdges = [];
	// 	const { item } = e;
	// 	const model = item.getModel();
	...
	// 			const itemz = graph.findById(snode.id);
	// 			graph.setItemState(itemz, "dark", false);
	...
	// 				const itemz = graph.findById(csedge.id);
	// 				if (itemz) graph.setItemState(itemz, "disappearing", true);
	// 			});
	// 		}
	// 		curShowNodes.forEach((csnode) => {
	// 			const itemz = graph.findById(csnode.id);
	// 			if (itemz) graph.setItemState(itemz, "disappearing", true);
	// 		});
	...
	// 			const layoutControllerz = graph.get("layoutController");
	// 			layoutControllerz.layoutCfg.nodeStrength = () => -80;
	// 			layoutControllerz.layoutCfg.collideStrength = 0.2;
	// 			layoutControllerz.layoutCfg.linkDistance = (d) => {
	// 				if (d.source.level !== 0) return 120;
	// 				const length = 250;
	// 				return length;
	// 			};
	// 			layoutControllerz.layoutCfg.edgeStrength = () => 2;
	...
	// 					let { color } = model;
	// 					if (nu.isLeaf) {
	// 						if (nv.level === 0 && nv.tag !== model.tag)
	// 							color = "#DFE5EB";
	// 						else if (!nv.isLeaf && nv.tags[0] !== model.tag) {
	// 							color = "#DFE5EB";
	// 						}
	// 					} else if (nv.isLeaf) {
	// 						if (nu.level === 0 && nu.tag !== model.tag)
	// 							color = "#DFE5EB";
	...
	// 			const edgeItems = graph.getEdges();
	// 			edgeItems.forEach((itemz) => {
	// 				graph.clearItemStates(itemz);
	// 			});
	// 			nodeItems.forEach((itemz) => {
	// 				graph.clearItemStates(itemz);
	// 				graph.setItemState(itemz, "appearing", true);
	// 			});
	// 		}, 400);
	// 	}
	// });
...
export default DecisionBubbles;
